...
import '../model/delete_req_model.dart';
...
class DashboardRepo {
  ...
  Future<Attempt<DeleteReqResponse>> getDelReqs() async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return failed(SessionExpired());
      ...
      final url = Uri.parse("$baseUrl/getDeleteRequests").replace();
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        print(jsonDecode(response.body));
        return success(
          DeleteReqResponse.fromJson(jsonDecode(response.body)),
        );
      } else if (response.statusCode == 401) {
        return failed(SessionExpired());
      } else if (response.statusCode == 403) {
